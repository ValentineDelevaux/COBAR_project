...
class PreprogrammedSteps:
    ...
    wings : List[str]
        List of wing names (LW for left wing).
    dofs_per_wings : List[str]
        List of names for degrees of freedom for each wing.
    duration : float
    ...
    dofs_per_wings = ["Wing_roll", "Wing_yaw", "Wing"]
    ...
    def get_wing_angles(self, phase, magnitude=0.2, speed_multiplier=7):
        ...
        wing_angle_R = np.array(
            [
                np.sin(phase + np.pi),  # roll
                np.cos(phase + np.pi),  # yaw
                0                       # pitch
            ]
        ) * magnitude
